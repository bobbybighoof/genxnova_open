...
float hit(float dist, float alphaIn, int i){
	...
	if(i < m_CollisionNum){
		  ...
		  alphaOut += pow(e,(-1.0*((x2-z)*(x2-z))*20.0))*(1.0-z)*2.0;
		  
		  alphaOut = max(0.0,min(1.0, alphaOut));
	}
	return alphaOut;
...
void main(void) {
	vec4 color = vec4(0.80, 0.80, 1.0, m_MinAlpha);
	...
	vec4 distort = texture2D(m_Distortion, noise.xy * gl_TexCoord[0].st/10.0); 	//load du/dv normalmap //+noise.xy/2.0
	


	color *= texture2D(m_ShieldTex, noise.xz*0.3+(distort.xy*0.1)+gl_TexCoord[0].st);
