...
void calculateLight(vec3 normal, vec3 VertexPosition){
  // figure angle to light
		...
		vec3 vFragColor = ambient + diffuse + spec;
		vAlpha = (vFragColor.x + vFragColor.y + vFragColor.z) * 0.0125;
		light = vec4((vFragColor * occlusion), 1.0);
}
...
void main()
{
	...
	#IFDEF virtual
		quad = vec2(mod(floor(mTex * 4.0), 2.0), mod(floor(mTex * 2.0), 2.0));
	#ELSE
	...
	if(sideId == 0.0){
		...
		calculateLight(vec3(0,-1,0), vPos);
		if(blockType == 0){
			texCoords = vec2( (1.0 - quad.x)*tiling, (quad.y)*tiling );
		}else{
			texCoords = vec2( (1.0 - quad.x)*tiling, (1.0 - quad.y)*tiling );
		}
