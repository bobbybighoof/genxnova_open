...
void main(void)
{
   ...
   vec4 diffuseColor = texture2D(baseMap, gl_TexCoord[0].st); 
   ...
   vec4 specularColor = texture2D(specMap, gl_TexCoord[0].st);
   
   vec4 specular = specularFactor * specularColor;
   
   vec4 diffuseFinal = diffuseColor * NdotL * fvDiffuse;
   
   vec2 cloudTexCoord = gl_TexCoord[0].st;
  ...
  //  	gl_FragColor.a *= fogFactor;
   ...
   //	gl_FragColor = vec4(simpleNormal,1.0);
}
